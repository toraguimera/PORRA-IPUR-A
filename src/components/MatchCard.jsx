...
export default function MatchCard({ match, isAdmin }) {
  ...
  const [adminPenaltyWinner, setAdminPenaltyWinner] = useState(null)
  ...
  async function saveAdminScore() {
    ...
    setSavingAdmin(true)
    const thePenaltyWinner = isKnockout && isAdminDraw ? adminPenaltyWinner : null
    const { error } = await supabase
      .from('matches')
      .update({
        ...
        status: 'finished',
        penalty_winner: thePenaltyWinner,
      })
      .eq('id', match.id)
    if (!error) {
      await recalculatePoints(parseInt(adminHomeScore), parseInt(adminAwayScore), thePenaltyWinner)
      setEditMode(false)
    }
    setSavingAdmin(false)
  }

  async function recalculatePoints(homeScore, awayScore, penaltyWinner) {
  ...
  function openEditMode() {
    setAdminHomeScore(String(match.home_score !== null ? match.home_score : ''))
    setAdminAwayScore(String(match.away_score !== null ? match.away_score : ''))
    setAdminPenaltyWinner(match.penalty_winner || null)
    setEditMode(true)
  }
    ...
    <div className={`bg-card rounded-2xl p-4 border ${isLive ? 'border-red-500' : saved && !hasStarted ? 'border-accent/50' : 'border-border'} mb-3`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          {match.label && !match.group && (
            <span className="text-xs font-bold bg-border px-2 py-0.5 rounded-full text-gray-300">{match.label}</span>
          )}
        </div>
        <div className="text-right">
          {isLive ? (
            <span className="text-xs font-bold text-red-400 flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-red-500 inline-block"/>EN DIRECTO
            </span>
          ) : isFinished ? (
          ...
          {isFinished || isLive ? (
            <div>
              <div className="flex items-center justify-center gap-2 text-3xl font-black">
                <span className={match.home_score > match.away_score ? 'text-white' : 'text-gray-400'}>{match.home_score}</span>
                <span className="text-gray-500 text-xl">-</span>
                <span className={match.away_score > match.home_score ? 'text-white' : 'text-gray-400'}>{match.away_score}</span>
              </div>
              {matchHasPenalties && (
      ...
      {isAdmin && ((!isFinished && !isPorDeterminar) || editMode) && (
        <div className="mt-3 pt-3 border-t border-border">
          ...
          <div className="flex items-center gap-2">
            <input type="number" min="0" max="99" value={adminHomeScore}
              onChange={(e) => { setAdminHomeScore(e.target.value); setAdminPenaltyWinner(null) }}
              className="score-input text-sm" placeholder="0" />
            <span className="text-gray-500">-</span>
            <input type="number" min="0" max="99" value={adminAwayScore}
          ...
          {isKnockout && isAdminDraw && (
            <div className="mt-2">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setAdminPenaltyWinner('away')}
                  className={`flex-1 py-2 text-xs font-bold rounded-xl border transition-all active:scale-95 ${adminPenaltyWinner === 'away' ? 'bg-gold text-black border-gold' : 'border-border text-gray-400 bg-transparent'}`}>
